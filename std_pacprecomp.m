% std_pacprecomp() - Compute PAC in the EEGLAB STUDY
%
% Usage:  
...
for iSubj = 1:length(uniqueSubjects)
    for iSess = 1:length(uniqueSessions)
        ...
        for ichan = 1:size(stream_indx,2)
            if strcmp(prefix,'chan')
                fieldname = [ prefix int2str(chan_indx(ichan))];
            else
                fieldname = [ prefix int2str(subjICs{iSubj}(ichan))];
            end
