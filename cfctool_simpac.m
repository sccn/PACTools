% cfctool_simpac() - Generate a signal with coupled phase and amplitude (PAC)
...
%                         
% Usage:
%   >> [data,simspk.t_out,simspk.phase_signal,simspk.m]  = cfctool_simpac(fc,fm,tlimits);
%   >> [data, t_out,phase_signal,m]  = cfctool_simpac(fc,fm,tlimits,'fs',srate,'cpfunc','block','blockamp',1,'plot_flag',1,'padtime',padtime,'snr',snrval);
% Inputs:
...
function [amplitude_mod, t,phase_signal,m] = cfctool_simpac(fc,fm,tlimits,varargin)
...
try
    ...
    disp('cfctool_simpac() error: calling convention {''key'', value, ... } error'); return;
end;
